...
def parse_inventory_report(report_bytes: bytes, marketplace_name: str,
    ...
    """
    Parse the FBA inventory planning report TSV into rows.

    GET_FBA_INVENTORY_PLANNING_DATA key columns:
    - snapshot-date
    - sku, fnsku, asin, product-name, condition
    - available                  — fulfillable/available quantity
    - Total Reserved Quantity    — reserved quantity
    - inbound-quantity           — total inbound
    - inbound-working            — inbound working
    - inbound-shipped            — inbound shipped
    - inbound-received           — inbound receiving
    - unfulfillable-quantity     — unsellable quantity
    """
    rows = []
    ...
    for row in reader:
        ...
        fulfillable = safe_int(row.get("available"))
        reserved = safe_int(row.get("Total Reserved Quantity"))
        inbound_working = safe_int(row.get("inbound-working"))
        inbound_shipped = safe_int(row.get("inbound-shipped"))
        inbound_receiving = safe_int(row.get("inbound-received"))
        unsellable = safe_int(row.get("unfulfillable-quantity"))

        # Use Amazon's pre-calculated inbound total if available,
        # otherwise sum the components
        total_inbound = safe_int(row.get("inbound-quantity"))
        if total_inbound is None:
        ...
        rows.append({
            ...
            "total_quantity": None,  # not provided by this report type
        })
...
def process_marketplace(conn, marketplace_name: str, marketplace_enum,
                        marketplace_id: str, snapshot_date: date):
    """Request, download, parse, and load inventory snapshot for one marketplace."""
...
def main():
    ...
    try:
        for i, (marketplace_name, marketplace_enum, marketplace_id) in enumerate(marketplaces):
            if i > 0:
                print("Pausing 60 seconds between marketplaces to avoid quota limits...")
                time.sleep(60)
            process_marketplace(
