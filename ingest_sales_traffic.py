...
@with_sp_api_retry(max_attempts=5, operation_name="create_report")
def _create_sales_traffic_report(reports_api, marketplace_id, start_date, end_date):
    """The actual SP-API call. Wrapped with retry/backoff."""
...
def request_sales_traffic_report(marketplace_name, marketplace_enum, marketplace_id, start_date, end_date):
    ...
    response = _create_sales_traffic_report(reports_api, marketplace_id, start_date, end_date)
...
@with_sp_api_retry(max_attempts=5, operation_name="get_report")
...
@with_sp_api_retry(max_attempts=5, operation_name="get_report_document")
...
def process_marketplace(conn, marketplace_name, marketplace_enum, marketplace_id, start_date, end_date) -> bool:
    """
    Returns True on success, False on failure.

    Failures are logged to the ingestion_job_log table and to stdout/log,
    but do NOT raise — so a failure in one marketplace does not block
    other marketplaces from running.
    """
    ...
    try:
        ...
        # Make sure the connection isn't left in an aborted-transaction state
        # before we try to write the failure log row.
        try:
            conn.rollback()
        except Exception:
            pass
        ...
        log.error(
            "[%s] Sales & Traffic ingestion FAILED: %s",
        ...
        # Do NOT re-raise. Returning False lets main() continue to the next
        # marketplace. The orchestrator will see the False and surface it
        # in the final summary.
        return False


# FIX #6: single connection, setup done once before processing any marketplace
# Configurable pause between marketplaces to avoid SP-API burst exhaustion.
INTER_MARKETPLACE_SLEEP_SECONDS = getattr(config, "INTER_MARKETPLACE_SLEEP_SECONDS", 60)


def main() -> dict[str, bool]:
    """
    Returns a dict mapping marketplace name -> success bool, e.g.
        {"US": True, "CA": False}
    Callers can use this to decide whether to continue with downstream steps.
    """
    print("=" * 60)
    ...
    try:
        ...
        for idx, (marketplace_name, marketplace_enum, marketplace_id) in enumerate(marketplaces):
            # Pause between marketplaces to avoid burst exhaustion on
            # SP-API endpoints with low rate limits like createReport.
            if idx > 0:
                sleep_with_log(
                    INTER_MARKETPLACE_SLEEP_SECONDS,
                    f"pacing before {marketplace_name} create_report call",
                )
