...
def process_marketplace(conn, marketplace_name: str, marketplace_enum, marketplace_id: str):
    """Request, download, parse, and load listings report for one marketplace."""
...
def main():
    ...
    try:
        # Step 1 — fetch listings report for each marketplace
        for marketplace_name, marketplace_enum, marketplace_id in marketplaces:
            process_marketplace(conn, marketplace_name, marketplace_enum, marketplace_id)

        # Step 2 — backfill SKU into dim_product
        backfill_sku_to_dim_product(conn)

        # Step 3 — backfill SKU into identity map
        backfill_sku_to_identity_map(conn)

        # Step 4 — print SKU coverage summary
    ...
    print("\n" + "=" * 60)
    print("Listings ingestion complete.")
    print("=" * 60)
