...
@with_sp_api_retry(max_attempts=4, operation_name="catalog.get_catalog_item")
def _get_catalog_item(catalog_api, asin: str, marketplace_id: str):
    """The actual SP-API call. Wrapped with retry/backoff."""
    return catalog_api.get_catalog_item(
        asin=asin,
...
def fetch_and_load_catalog(conn, marketplace_name: str, marketplace_enum, marketplace_id: str):
    ...
    for i, asin in enumerate(asins, 1):
        try:
            response = _get_catalog_item(catalog_api, asin, marketplace_id)
...
def main() -> dict[str, bool]:
    """
    Returns a dict mapping marketplace name -> success bool.
    A marketplace is considered "successful" if its catalog fetch
    completed without an unhandled exception. Per-ASIN failures
    do not flip the marketplace to failed.
    """
    print("=" * 60)
    ...
    try:
        # Step 1 — fetch catalog data for all active ASINs (isolated per marketplace)
        for marketplace_name, marketplace_enum, marketplace_id in marketplaces:
            try:
                ...
                results[marketplace_name] = True
            except Exception as exc:
                log.error(
                    "[%s] Catalog fetch FAILED at marketplace level: %s",
        ...
        # Otherwise, dim_product would be rebuilt from incomplete staging.
        if any(results.values()):
            try:
                ...
                log.error("Catalog post-processing FAILED: %s", exc, exc_info=True)
                try:
                    conn.rollback()
                except Exception:
                    pass
        else:
            log.warning("Skipping dim_product / identity map build — all marketplaces failed.")
